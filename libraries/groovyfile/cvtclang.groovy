void call(){
    ...
    node {
        currentBuild.description = 'Make convert clang by groovy'
        currentBuild.result = 'SUCCESS'
        def cvtclang_content = config.cvtclang_content ?:
			""
        def makexml_content = config.makexml_content ?:
			""
        try {
            stage("$currentBuild.description Stage"){
                echo "Start $currentBuild.description "
				def cvtClang, makeXml
				if (cvtclang_content.length() > 0) {
					cvtClang = cvtclang_content
				} else {
					cvtClang = this.getAddClang()
				}
				if (makexml_content.length() > 0) {
					makeXml = makexml_content
				} else {
					makeXml = this.getMakeXml()
				}
				writeFile file: "${WORKSPACE}/cvtclang.py", text: "${cvtClang}"
				writeFile file: "${WORKSPACE}/makexml.j2", text: "${makeXml}"
            }
        } catch (e) {
...
def covertReserved(inp):
    ...
    out = out.replace("&", "&amp;")
    out = out.replace("%", "&#37;")
    out = out.replace('&slash&&slash&&slash&"', "&quot;")
    return out
...
def main(args):
    ...
    p = unicode(te.render_output('makexml.j2', jsondata).replace('&slash&'','&apos;').replace('(&slash&")', '(&quot;)'))
    print p
