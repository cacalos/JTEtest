...
int convert_to_smsfif_msg(smsf_msg_t *smsf_msg, smsf_send_t *smsf_sendmsg, int *len, char *supi)
{
	smsf_sendmsg->type = 2;
	*len = sizeof(smsf_sendmsg->type);
	...
	smsf_sendmsg->causeCode = 0;
	*len += sizeof(smsf_sendmsg->causeCode);

	smsf_sendmsg->supi_len = strlen(supi);
...
//	sprintf(smsf_sendmsg->supi, "%s", "530000000000000");
	memcpy(smsf_sendmsg->supi, supi, smsf_sendmsg->supi_len);
	*len += sizeof(smsf_sendmsg->supi_len) + smsf_sendmsg->supi_len;

	smsf_sendmsg->gpsiLen = strlen("01044445555");
...
//	memcpy(smsf_sendmsg->gpsi, &smsf_msg->gpsi, smsf_sendmsg->gpsiLen);
	*len += sizeof(smsf_sendmsg->gpsiLen) + smsf_sendmsg->gpsiLen;
	...
	if(smsf_sendmsg->contentDataLen != 0) {
		...
		smsf_sendmsg->contentData[0]= 3;
		smsf_sendmsg->contentData[1] = 15;
//		smsf_sendmsg->contentData[2] = 1;
//		smsf_sendmsg->contentData[3]= 111;
...
//	uxc_ipcmsg_t        *rcv_msg = (uxc_ipcmsg_t *)msg->data;
	...
	ret = convert_to_smsfif_msg(smsf_msg, &smsf_sendmsg, &len, supi);
	if(ret != IXOK) 
	{
		ux_log( UXL_CRT, "MakeMoResponse Fail[%s]", supi);
		return IXERR;
	}
	...
	if(ret < UX_SUCCESS)
	{
		ux_log( UXL_CRT, "upa_tcp_send2() fail, [%d], Type[%d], CC[%d], SUPI[%s], GPSI[%s], DataLen[%d]",	
				msg_size,
				smsf_sendmsg.type, 
	...
	{
		ux_log( UXL_INFO, "upa_tcp_send2() success, TotalLen[%d], Type[%d], CC[%d], SUPI[%s], GPSI[%s], DataLen[%d]",
				msg_size,
				smsf_sendmsg.type, 
				smsf_sendmsg.causeCode,
				smsf_sendmsg.supi,
